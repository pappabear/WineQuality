...
Without any special data treatment, we see that random forest provides a model with *87.1%* accuracy, the best of the algorithms chosen.  This is the baseline model, further attempts for improvement follow.

Baseline: Random Forest yields 87.1% accuracy, with no treatment for outliers and no feature engineering.

Next step: try dealing with outliers.

***
#### Try to improve accuracy using caret's preProcess() function
...
```{r}
dataset.training.original <- dataset.training
...
```{r}
# return the training dataset to the original state
dataset.training <- dataset.training.original
...
# train random forest
model.rf = train(quality ~ ., 
                 data=dataset.training[c(11,2,10,7,4,12)], 
                 method='rf', 
                 tuneLength=5, 
                 trControl = fit.control)
...
***
##### Use Principal Component Analysis to reduce the number of predictors in the model

```{r}
pca <- prcomp(dataset.training[1:11], scale. = T)
biplot(pca, scale = 0)
```

```{r}
# determine the proportion of variance explained by each variable
variance.proportion <- pca$sdev^2

# use scree plot to determine how many variables to use in modeling
plot(variance.proportion, xlab = "Principal Component",
...
plot(cumsum(variance.proportion), xlab = "Principal Component",
              ylab = "Cumulative Proportion of Variance Explained",
              type = "b")
```

Analysis shows the top 8-10 predictors explain most of the variance in the dataset. 

Re-modeling with those variables may improve the accuracy.


```{r}
# build the training dataset from PCA
dataset.training.pca <- data.frame(quality = dataset.training$quality, pca$x)
dataset.training.pca <- dataset.training.pca[,1:10]
dataset.training.pca
```
...
Through experimentation using RCA to determine the top predictors, the model accuracy could be improved to 87.4% using the top 9 features in the dataset.
